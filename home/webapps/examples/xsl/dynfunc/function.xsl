...
<xsl:stylesheet 
  ...
  <xsl:template name="title" as="xs:string">Example 21: Dynamic/scripted extension functions, example</xsl:template>
  ...
  <xsl:template name="tab-contents-1">
    ...
    <xsl:variable name="result" select="dynfunc:register($code-units)" as="element(dynfunc:diagnostics)?"/>
    
    <!-- Check the registration result: -->
    <xsl:if test="$result/dynfunc:diagnostic/@kind = 'ERROR'">
    ...
    <xsl:try>
      <p>
        ...
        <xsl:sequence select="dynfunc:call(QName($function-ns-uri, 'lcs'), 'ABACCD', 'ACDF')"/>
        <hr/>
      </p>
      <p>
      ...
      <p>
        ...
        <xsl:sequence select="dynfunc:call(QName($function-ns-uri, 'implicit-objects'), 'Hello World')"/>
        <hr/>
      </p>
      <xsl:catch>
        <p>
