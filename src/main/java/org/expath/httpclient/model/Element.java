...
/**
 * An abstract representation of an element (just provide the HTTP Client
 * needs).
 * 
...
public interface Element {
   ...
   * 
   * The display name is the original lexical name (with the original prefix if
   * any). An implementation is not required to return the exact same name as
   * the original, and can instead make a "best guess". This is for reporting
   * purpose only.
   */
   ...
   * 
   * @param name
   *          The local name of the attribute to look for. The attribute is
   *          looked for in no namespace.
   * 
  ...
  /**
   * Check the element {@code elem} does not have attributes other than
   * {@code names}.
   * 
   ...
   * 
   * @param names
   *          The non-qualified names of allowed attributes (cannot be null, but
   *          can be empty.)
   * 
   * @throws HttpClientException
   *           If the element contains an attribute in the HTTP Client
   *           namespace, or in no namespace and the name of which is not in
   *           {@code names}.
   */
