...
public class NodesManager {
   ...
   *
   * What we want is: - a direct DiffID <-> Node mapping - a path from v1 Node
   * to its hash value - a path from an hash value to v0 nodes with the same
   * hash - a notion of 'weight' for a subtree in v1
   *
   */
  public void registerSourceDocument(TreeNode sourceDoc) throws Exception {
  ...
  public void registerResultDocument(TreeNode resultDoc) throws Exception {
    if (this.sourceNumberOfNodes == 0) {
  ...
  private int registerSubtree(TreeNode node, boolean isSource) throws Exception {
    if (node == null) {
  ...
  private void computeCandidateIndexTables(int v0nodeID) throws Exception {
  ...
  private void nodeAssign(int v0nodeID, int v1nodeID) throws Exception {
  ...
  private boolean tryNodeAssign(int v0nodeID, int v1nodeID) throws Exception {
  ...
  private void forceParentsAssign(int v0nodeID, int v1nodeID, int level) throws Exception {
   ...
   *
   * The algorithm relies on two features: 1) Order the nodes to match in
   * top-down or weight-related order 2) Find the best node among a certain
   * number of nodes with the same content
   *
   */
  public void topDownMatch(int v0rootID, int v1rootID) throws Exception {
  ...
  private void recursiveAssign(int v0nodeID, int v1nodeID) throws Exception {
    if ((v0nodeID == 0) || (v1nodeID == 0)) {
  ...
  private int getBestCandidate(int v1nodeID, int selfkey) throws Exception {
  ...
  /*
   * Optimization Phase * * The goal of this phase is to use matchings, and if
   * they give us * obvious results for children *
   */
  ...
  /*
   * create the Script that constructs v1 from v0. This script will then be used
   * to create the XML-delta document
   *
   */
