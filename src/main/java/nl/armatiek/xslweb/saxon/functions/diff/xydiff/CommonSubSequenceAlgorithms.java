...
public class CommonSubSequenceAlgorithms {
   ...
   *
   * The Goal of a Common SubSequence Algorithm is, given to input Sequences, to
   * find a subsequence that exists in both of them There may be many solutions,
   * and various algorithms will try to find the longest common subsequence, or
   * the subsequence that has the most important value (if sequence elements are
   * given a weight)
   *
   ...
   * version of the document, their position may have changed. We ignore
   * position changes due to operation on their sibling. Still, some nodes may
   * have been reordered, for example nodes 1, 2, 3 may become nodes 2, 3, 1
   * Finding the longest common subsequence (here 2, 3) gives us the minimal set
   * of move operations that transforms the old sequence of children into the
   * new one. Here: <move node 1 from position 1 to position 3>
   *
   *
   * Our implementation of Longest Common SubSequence has a time cost in n1*n2,
   * where n1 and n2 are the length of both input sequences The subsequence we
   * identify is the 'longest' if the weight is the same for all children, but
   * in a more general weight is the subsequence which has the most important
   * weight. So the set of operations that transforms the old list of children
   * into the new one is the 'smallest' set of operations, e.g. in general the
   * set of operations with the lowest weight.
   *
   ...
   *
   *
   * USAGE: INPUT-1: S1[1]...S1[S1.size()-1] represent the first sequence (note
   * that S1[0] is ignored) They must be equal to 1, 2, 3, ..., S1.size()-1
   * (because of easy_lcss implementation) INPUT-2: S2[1]...S2[S2.size()-1]
   * represent the second sequence (note that S2[0] is ignored)
   *
   * RESULT: Both sequences are modified, in that some element values are set to
   * 0. This mean that the element is NOT part of the subsequence. For all other
   * elements, the value has not changed, and the element is part of the
   * identified common subsequence.
   *
   */

  // translation done by Tani
  // s1 and s2 are vector of WSequence
  public static void easy_css(Vector<WSequence> s1, Vector<WSequence> s2) {
    int cursor1 = 1;
    int cursor2 = 1;
    ...
    while ((cursor1 < s1.size()) && (cursor2 < s2.size())) {
      if (((WSequence) s1.get(cursor1)).getData() == 0) {
        ...
        if (((WSequence) s2.get(cursor2)).getData() == 0) {
          // System.out.println("s2[" + cursor2 + "] is already removed ... but
  ...
  public static void lcss(Vector<WSequence> s1, Vector<WSequence> s2) {
    // Init
    int l1 = s1.size();
    int l2 = s2.size();
    ...
    for (int j = 1; j < l2; j++) {
      for (int i = 1; i < l1; i++) {
        ...
        // Delete item i in S1 and use transformation of S1[1..i-1] into
        // S2[1..j]
