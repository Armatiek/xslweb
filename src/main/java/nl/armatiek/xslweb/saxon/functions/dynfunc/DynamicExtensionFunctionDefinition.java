...
public class DynamicExtensionFunctionDefinition extends ExtensionFunctionDefinition {
  ...
  private void throwConversionException(AtomicValue value, Class javaClass) throws XPathException {
    throw new XPathException("Conversion from \"" + value.getItemType().getDisplayName() + "\" to \"" + javaClass.toString() + "\" is not supported", "DF012");
  }
  ...
  private Object convertToJava(Item item, Class javaClass) throws XPathException {
    if (item == null) {
      ...
      if (!(NodeInfo.class.isAssignableFrom(javaClass))) {
        throw new XPathException("Conversion from \"" + item.getClass().toString() + "\" to \"" + javaClass.toString() + "\" is not supported", "DF012");
      }
      Object node = item;
      ...
      case StandardNames.XS_UNTYPED_ATOMIC:
        ...
        throwConversionException(value, javaClass);
