...
NOTE: The information below (including section 14.6) does only apply to XSLWeb version 4.1 or the master of XSLWeb git.
...
IMPORTANT: Byte sizing of heap capacity currently does not work within XSLWeb (investigation is ongoing...). Therefore it is recommended for now to only specify the capacity of heap memory in number of entries, not in bytes. 
