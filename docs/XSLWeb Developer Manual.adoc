...
* _config:development-mode_: whether the webapp runs in development mode (true()) or production-mode (false()).
* _config:debug-mode_: whether the webapp runs in debug mode or not.
* _config:cache-buster-id_: see <<cache-busting>>.
* _req:request-xml-doc_: the to XML serialized request object, which is also the input document of every pipeline (req = _\http://www.armatiek.com/xslweb/request_)
